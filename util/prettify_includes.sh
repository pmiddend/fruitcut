...
#	find src/fruitapp src/fruitlib src/sandbox src/test include/fruitapp include/fruitlib \( -name '*.hpp' -o -name '*.cpp' \) | xargs $0 run
	find src \( -name '*.hpp' -o -name '*.cpp' \) | xargs $0 run
fi
